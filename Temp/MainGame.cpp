...
void HandlePlayerControls(); //Declaring the function here allieviates issues the compiler not recognising function names if they are further down in the compiler. 
void UpdateFan();
void UpdateTools();
...
bool MainGameUpdate(float elapsedTime)
{
	...
	//When return is true the exit method is called
...
void UpdateFan()
{
	...
	if (Play::RandomRoll(50) == 50)
	{
		...
		if (Play::RandomRoll(2) == 1) //Chance of a tool spawning as a spanner instead of a screwdriver
		{
	...
	if (Play::RandomRoll(150) == 1) //Chance of spawning a coin. Not mutually exclusive with spawning a screwdriver/spanner
	{
...
void UpdateTools()
{
	...
	for (int id : vTools)
	{
		...
		if (gameState.agentState != STATE_DEAD && Play::IsColliding(obj_tool, obj_agent8)) //Tool collision kills the player
		{
...
void UpdateCoinsAndStars()
{
	...
	for (int id_coin : vCoins)
	{
		...
		{
			//Spawns multiple star particles at each diagonal.
			for (float rad{ 0.25f }; rad < 2.0f; rad += 0.5f) //Somewhat confused on the significance of the curly braces in this, since it is a float type, but I think its to prevent narrowing
			{
